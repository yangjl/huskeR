...
#'
#'
#' @param inputdf An input data.frame to plot. plot_mht: [df, =col2plot, "chr", "pos"]; plot_jmht: [,cex].
...
#' @rdname plot_mht
plot_jmht <- function(inputdf, col2plot="ModelFreq",
                     cl_file = "~/bin/zmSNPtools/Rcodes/chr_length_B73v3.csv",
                     cex=.9, pch=16, col=rep(c("slateblue", "cyan4"), 5),
                     GAP=5e+06, yaxis=NULL,
                     ... ){
    ...
    for(i in 1:10){
        points(x=subset(res, chr==i)$newpos, y=res[res$chr==i, col2plot],
               pch = pch, col=col[i], cex=res[res$chr==i, ]$cex);
    }
}
